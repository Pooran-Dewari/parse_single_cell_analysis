...
# Create the gene_map2 dataframe and clean up the gene symbols (remove spaces, periods, hyphens, and other special characters)
gene_map2 <- cg_science %>%
  select(gene_id = Gene.ID, gene_symbol = Gene.symbol.science) %>%
  mutate(
    gene_symbol = gsub("[^[:alnum:]]", "", gene_symbol)  # Remove all non-alphanumeric characters
  )
