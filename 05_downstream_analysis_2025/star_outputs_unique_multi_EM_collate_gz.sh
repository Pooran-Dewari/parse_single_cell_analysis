...
final_dir="./star_outputs_Uni_Mult_EM_collated"
...
for dir in a*_resultsSolo.out; do
    ...
    for file in "${files_to_copy[@]}"; do
        ...
        if [ $? -eq 0 ]; then
            ...
            if [ $? -eq 0 ]; then
                echo "Compressed $file to ${dest_file}.gz successfully."
                echo "Compressed $file to ${dest_file}.gz successfully." >> $logfile
            else
                echo "Failed to compress $file."
                echo "Failed to compress $file." >> $logfile
            fi
        else
    ...
    # Copy and rename UniqueAndMult-EM.mtx to matrix.mtx
    src_matrix="$sub_dir/UniqueAndMult-EM.mtx"
    ...
    if [ $? -eq 0 ]; then
        echo "Copied and renamed $src_matrix to $dest_matrix successfully."
        echo "Copied and renamed $src_matrix to $dest_matrix successfully." >> $logfile

        # Compress the copied matrix file
        gzip -f "$dest_matrix"

        if [ $? -eq 0 ]; then
            echo "Compressed matrix.mtx to ${dest_matrix}.gz successfully."
            echo "Compressed matrix.mtx to ${dest_matrix}.gz successfully." >> $logfile
        else
            echo "Failed to compress matrix.mtx."
            echo "Failed to compress matrix.mtx." >> $logfile
        fi
    else
        echo "Failed to copy and rename $src_matrix."
        echo "Failed to copy and rename $src_matrix." >> $logfile
    fi
